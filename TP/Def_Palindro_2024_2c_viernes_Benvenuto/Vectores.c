...
void mostrarDatosDefensa(const void* e1)
{
    const DefensaPal* palabra = e1;
    printf("%02d|%-50s",palabra->cant,palabra->pal);
}

int cmpPalabra(const void* e1, const void* e2)
{
    const DefensaPal* Palabra1 = e1;
    const DefensaPal* Palabra2 = e2;
    int comp = strcmp(Palabra1->pal,Palabra2->pal);
